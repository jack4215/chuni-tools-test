    ...
    function Yr(t) {
      ...
      return {
        ...
        p(e, [t]) {
          ...
          if (e[0].newV) {
            if (!newVIndicator) {
                newVIndicator = Vz(T); // 使用新數據
                newVIndicator.c(); // 創建 DOM 結構
                newVIndicator.m(u, null); // 插入到 <td> 中
                f.textContent = ""; // 清空原始數值
            }
        } else if (newVIndicator) {
            // 如果 newV 從存在變為不存在
            newVIndicator.d(1); // 移除 newV-indicator
            newVIndicator = null;
            f.textContent = T; // 恢復原始數值到 <td>
        }
        },
        i: e,
        o: e,
