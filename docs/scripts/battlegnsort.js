...
const boxes = document.querySelectorAll('.box05.w400');
const firstBox = boxes[0];

if (firstBox) {
    for (let i = 1; i < boxes.length; i++) {
        const musicBoxes = boxes[i].querySelectorAll('.music_box');
        musicBoxes.forEach(musicBox => firstBox.appendChild(musicBox));
        boxes[i].remove();
    }
}

const scrollPoints = document.querySelectorAll('.genre.scroll_point');
scrollPoints.forEach(scrollPoint => scrollPoint.remove());

function sortMusicBoxes(desc = false) {
    const musicBoxes = Array.from(firstBox.querySelectorAll('.music_box'));
