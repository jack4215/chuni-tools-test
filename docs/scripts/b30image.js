/* (c) 2021 chunirec */ ! function(e) {
    ...
    let n = !1,
        ...
        E = {
            ja: ["chunirec は現在、このバージョンには対応していません。", "CHUNITHM-NET にログインして Aime カードを選択してください。", "接続に失敗しました。", "chunirec スコア更新ブックマークレット", "chunirec との疎通確認中", "しばらくお待ちください。", "CHUNITHM-NET に接続中...", "プレイデータを取得しています", "レコードを取得中...", "コースレコードを取得中...", "レーティング情報を取得中...", "プレイヤー情報を取得中...", "送信中...", "chunirec は現在メンテナンス中です", "プレイデータの取得を開始します。", "続行", "中止", "既に実行中です。", "<< 戻る", "全国対戦戦績を取得中...", "セッションが切れたか、ゲキチュウマイ-NET 利用権のスタンダードコースが適用されていない可能性があります。", "<div style='line-height: 1.5;'>2023 年 2 月 1 日をもちまして、「CHUNITHM PARADISE LOST」のスコア登録機能の提供は終了しました。ご利用ありがとうございました。<br><br><strong>chunirec は「CHUNITHM NEW」以降のバージョンに対応しています。</strong></div>", "CHUNITHM PARADISE LOST には対応していません"],
            en: ["Chunirec hasn't been able to work on this version yet.", "Sign in to your CHUNITHM-NET account and select an Aime card.", "Failed to connect.", "Chunirec Score Updater", "Checking the connection to Chunirec", "Please wait a moment.", "Connecting to CHUNITHM-NET...", "Collecting your play data", "Collecting records...", "Collecting course records...", "Collecting rating info...", "Collecting player profile...", "Sending...", "Chunirec is down for maintenance", "Click [Continue] to start extracting your play data.", "Continue", "Stop", "This bookmarklet is already running.", , "<< Back", "Collecting net-battle result...", "", "<div style='line-height: 1.5;'>2023 年 2 月 1 日をもちまして、「CHUNITHM PARADISE LOST」のスコア登録機能の提供は終了しました。ご利用ありがとうございました。<br><br><strong>chunirec は「CHUNITHM NEW」以降のバージョンに対応しています。</strong></div>", "CHUNITHM PARADISE LOST には対応していません"]
        },
    ...
    function X(e) {
        H(R(11), R(7)), me(void 0, O("home/playerData/"), null, function(n, t) {
            if (n) {
                ...
                for (let e = 0; e < 3; ++e) {
                    if (!r[e]) continue; // 若 r[e] 是 undefined，跳過該迴圈
    ...
    function he(e, n) {
        ...
        ["/", "/mobile/", "/mobile/aimeList/", "/mobile/loginNewsAll/", "/mobile/error/", "/chuni-mobile/html/mobile/", "/chuni-mobile/html/mobile/aimeList/", "/chuni-mobile/html/mobile/loginNewsAll/", "/chuni-mobile/html/mobile/nError/"].includes(e) ? D(R(1), "110003") : (k(), H("", R(3)), "jp2" === l ? Y([Q, K, $, J, F, X, B, G, z]) : "jp_lim" === l && Y([Q, K, Z, J, q, W, G, z]))
    }()
}(document);